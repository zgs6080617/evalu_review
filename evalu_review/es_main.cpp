				...
				case STATIS_MODEL:
					{
						...
						ret = s_model.getHHStatusForUp(&es_time);
						if (ret < 0) {
							flag = 1;
						}

						ret = s_model.DeleteHHForUp(&es_time);
						if (ret < 0) {
							flag = 1;
						}
...
int CaculateAvgAll(int statis_type, ES_TIME *es_time)
{
	...
	sprintf(sql, "insert into result.%s(organ_code,code,value,datatype,updatetime,statistime,dimname,dimvalue) "
		"select a.organ_code,a.code+'Aver',round(a.v*0.95+b.v*0.05,4),a.d,sysdate,sysdate-1,'无','无' from "
		"(select organ.code,organ.organ_code organ_code,decode(a.value, NULL, 0, a.value) v,decode(a.datatype, NULL, '百分比',"
		"a.datatype) d from (select * from result.%s where datatype in('百分比', '数值') and dimname = '无') a "
		"right join(select b.organ_code, a.code from(select * from evalusystem.config.averall where flag = 0 and code_type = 0) a, "
		"(select * from config.organ where flag = 1 and subtype = 0) b) organ on (a.organ_code = organ.organ_code "
		"and a.code = organ.code)) a, "
		"(select organ.code,organ.organ_code,decode(b.v,NULL,0,b.v) v ,decode(b.d,NULL,'百分比',b.d) d from "
		"(select a.code code,substr(a.organ_code, 0, 5) organ_code,"
		"round(avg(a.value), 4) v,a.datatype d from(select * from result.%s where datatype in('百分比', '数值') "
		"and dimname = '无') a, (select * from evalusystem.config.averall where flag = 0 and code_type = 0) b, (select * from config.organ "
		"where flag = 0) c where a.code = b.code and a.organ_code = c.organ_code group by substr(a.organ_code, 0, 5),"
		"a.code,a.datatype) b right join(select b.organ_code, a.code from(select * from evalusystem.config.averall where "
		"flag = 0 and code_type = 0) a, (select * from config.organ where flag = 1 and subtype = 0) b) organ on(b.organ_code = organ.organ_code "
		"and organ.code = b.code)) b "
		"where a.code = b.code and a.organ_code = b.organ_code and a.d = b.d;", table_name, table_name, table_name);
...
#endif
	sprintf(sql, "insert into result.%s(code,organ_code,value,datatype,updatetime,statistime,dimname,dimvalue) "
		"select b.code+'Aver',b.organ_code,decode(a.value,NULL,0,a.value),decode(a.datatype,NULL,'百分比',a.datatype) "
		", sysdate, sysdate - 1, '无', '无' from "
		"(select * from result.%s where datatype in('百分比', '数值') and dimname = '无') a right join "
		"(select b.organ_code, a.code from(select * from evalusystem.config.averall where flag = 0 and code_type = 0) a, "
		"(select * from config.organ where flag = 1 and subtype = 1) b) b on(a.code = b.code and "
		"a.organ_code = b.organ_code) ;", table_name, table_name);
...
#endif
	sprintf(sql, "insert into result.%s(code,organ_code,value,datatype,updatetime,statistime,dimname,dimvalue) "
		"select b.code+'Aver',b.organ_code,decode(a.value,NULL,0,a.value),"
		"decode(a.datatype, NULL, '百分比', a.datatype), sysdate, sysdate - 1, '无', '无' from "
		"(select * from result.%s where datatype in('百分比', '数值') and dimname = '无') a right join "
		"(select b.organ_code, a.code from(select * from evalusystem.config.averall where flag = 1 and code_type = 0) a,"
		"(select * from config.organ where flag = 1) b) b on(a.code = b.code and a.organ_code = b.organ_code);",
		table_name, table_name);
...
int CaculateIndexDate( ES_TIME *es_time)
{
	...
	/* // @detail 插入前做一次删除操作，避免重复 */
	sprintf(query,"DELETE FROM EVALUSYSTEM.RESULT.DEVICE_STATIS WHERE COUNT_TIME = TO_DATE('%04d%02d%02d','YYYY-MM-DD');",
		pyear, pmonth, pday);
	...
	for (; it_organ != organ_temp.end();++it_organ) {
		...
		memset(query,0x00,sizeof(query));
		sprintf(query, "INSERT INTO EVALUSYSTEM.RESULT.DEVICE_STATIS(ID,ORGAN_CODE,KG_COUNT,KG_AVG,PB_COUNT,PB_AVG,DZ_COUNT,"
			"DZ_AVG,MX_COUNT,MX_AVG,ZF_COUNT,ZF_AVG,GZ_COUNT,GZ_AVG,EY_COUNT,EY_AVG,SY_COUNT,SY_AVG,TE_COUNT,TE_AVG,LINE_NUM,COUNT_TIME,UPDATETIME) "
			"VALUES(%d,%d,"
			"(select count(*) as value from detail.gpmscb where type='1' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d where code = 'breakfineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select count(*) from detail.gpmsld where organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d where code = 'ldfineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select count(*) from detail.gpmscb where  type='0' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d  where code = 'disfineAver' and dimname='无' and datatype='数值'and organ_code = %d),"
			"(select count(*) from detail.gpmsbus where organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d where code = 'busfineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select count(*) from detail.gpmsst where organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d  where code = 'substionfineAver'  and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select sum(value) as value  from RESULT.DAY_%04d%02d%02d where code='indicatornum' and dimname='无' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d where code = 'incovlineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select sum(value) as value  from RESULT.DAY_%04d%02d%02d where code='tworemotenum' and dimname='无' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d where code = 'twocovlineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select sum(value) from RESULT.DAY_%04d%02d%02d where code='threeremotenum' and dimname='无' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d  where code = 'threecovlineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select sum(value) as value from RESULT.DAY_%04d%02d%02d where (code='indicatornum' or code = 'tworemotenum' or code = 'threeremotenum') and dimname='无' and organ_code like '%d%%'),"
			"(select avg(value) from RESULT.DAY_%04d%02d%02d  where code = 'tercovlineAver' and dimname='无' and datatype='数值' and organ_code = %d),"
			"(select sum(value) from RESULT.DAY_%04d%02d%02d  where code = 'linenum' and dimname='无' and datatype='个数' and organ_code like '%d%%'),"
			"to_date('%04d%02d%02d','yyyy-mm-dd'),sysdate );",
			*it_organ, *it_organ,
			*it_organ,
...
#define TEST_UPDATE 1
	...
	/// @detail 两周整改率指标计算
	ret = s_support.GetCorrectRate(STATIS_DAY, es_time);
	if (ret < 0) {
	} // 
#endif
#if TEST_YC
	...
	/// @detail 升级正确率指标 */
	printf("升级完整率指标统计\n");
	EquipmentUpgrade s_EquipmentUpgrade;
	s_EquipmentUpgrade.Operator(STATIS_DAY, es_time);
#endif // TEST_UPDATE
#endif
	...
	DeleteStatisData(STATIS_DAY, es_time);//删除'数值'重复数据
	/*CaculateIndexDate(es_time);*/ //只读取日指标数据
	//月统计
#if TEST_DMS
	//主站运行情况
	printf("月统计\n");
	...
	ret = s_support.GetCorrectRate(STATIS_MONTH, es_time);
	if (ret < 0) {
	} // 
#endif
#if TEST_YC
	...
	printf("升级完整率指标统计\n");
	//EquipmentUpgrade s_EquipmentUpgrade;
	s_EquipmentUpgrade.Operator(STATIS_MONTH, es_time);
#endif // TEST_UPDATE
#endif
...
int TruncateGisUpdateData()
{
	...
	CDci *m_oci = new CDci;
	ret_ = m_oci->Connect(DATABASE_NAME, USERNAME, PASSWORD, &err_info);
	if (!ret_) {
		printf("Connect: error(%d) = %s;\n", err_info.error_no, err_info.error_info);
		return -1;
	}
	...
	sprintf(sql_delete, "TRUNCATE TABLE EVALUSYSTEM.DETAIL.GPMSLDUP;");
	ret_ = m_oci->ExecSingle(sql_delete, &err_info);
	if (!ret_) {
		printf("ExecSingle: error(%d) = %s, sql=%s;\n", err_info.error_no, err_info.error_info, sql_delete);
	}

	memset(sql_delete, 0x00, sizeof(sql_delete));
	sprintf(sql_delete, "TRUNCATE TABLE EVALUSYSTEM.DETAIL.GPMSSTUP;");
	ret_ = m_oci->ExecSingle(sql_delete, &err_info);
	if (!ret_) {
		printf("ExecSingle: error(%d) = %s, sql=%s;\n", err_info.error_no, err_info.error_info, sql_delete);
	}
	...
	memset(sql_delete, 0x00, sizeof(sql_delete));
	sprintf(sql_delete, "TRUNCATE TABLE EVALUSYSTEM.DETAIL.GPMSCBUP;");
	ret_ = m_oci->ExecSingle(sql_delete, &err_info);
	if (!ret_) {
		printf("ExecSingle: error(%d) = %s, sql=%s;\n", err_info.error_no, err_info.error_info, sql_delete);
	}
	...
	memset(sql_delete, 0x00, sizeof(sql_delete));
	sprintf(sql_delete, "TRUNCATE TABLE EVALUSYSTEM.DETAIL.GPMSBUSUP;");
	ret_ = m_oci->ExecSingle(sql_delete, &err_info);
	if (!ret_) {
		printf("ExecSingle: error(%d) = %s, sql=%s;\n", err_info.error_no, err_info.error_info, sql_delete);
	}

	ret_ = m_oci->DisConnect(&err_info);
	if (!ret_) {
		printf("DisConnect: error(%d) = %s;\n", err_info.error_no, err_info.error_info);
		return -1;
	}
