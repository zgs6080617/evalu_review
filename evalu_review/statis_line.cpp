...
int STATISLINES::ReadDmsLinesByDay( ES_TIME *es_time )
{
	...
	//挂接终端线路总条数
	sprintf(sql, "SELECT ORGAN_CODE,COUNT(*) FROM (SELECT DISTINCT ORGAN_CODE,LINEID FROM DETAIL.DMSLINE WHERE "
		"COUNT_TIME = '%04d-%02d-%02d' AND (THREERATIO != 0 OR INDICATOR != 0 OR TWORATIO != 0)) GROUP BY ORGAN_CODE;",
		byear, bmonth, bday);
	...
	if (ret == 1) {
		...
		for (rec = 0; rec < rec_num; rec++) {
			...
			m_mOrganCode2HaveTerNum.insert(make_pair(da.organ_code, da.count));
...
int STATISLINES::ReadDmsLinesByMonth( ES_TIME *es_time )
{
	...
	for (int i = 0; i < days; i++) {
		...
		if (i == days - 1) {
			sprintf(tmp_sql, "select * from evalusystem.result.day_%04d%02d%02d where dimname = '无' and datatype = '百分比' "
				" and code = 'tercovline' ", byear, bmonth, i + 1);
			tmp_str += tmp_sql;
		}
		else {
			sprintf(tmp_sql, "select * from evalusystem.result.day_%04d%02d%02d where dimname = '无' and datatype = '百分比' "
				" and code = 'tercovline' union all ", byear, bmonth, i + 1);
			tmp_str += tmp_sql;
		}
	}

	sprintf(sql, "select organ_code,avg(value) from (%s) where code='tercovline' and dimname='无' and datatype = '百分比' "
		"group by organ_code", tmp_str.c_str());
	...
	if (ret == 1) {
		...
		for (rec = 0; rec < rec_num; rec++) {
			...
			m_mOrganCode2TerResult.insert(make_pair(da.organ_code, da.count));
#ifdef DEBUG
			outfile << ++line << "\tdv:" << da.dv << "\torgan_code:" << da.organ_code << "\tgtime:" << da.count << endl;
#endif
		}
...
int STATISLINES::CaculateLineRate()
{
	...
	for (; it_line != m_mlines.end(); ++it_line) {
		...
		it = m_mOrganCode2HaveTerNum.find(it_line->first);
		if (it != m_mOrganCode2HaveTerNum.end()) {
		...
		m_mOrganCode2TerResult.insert(make_pair(es_result.m_ntype, es_result.m_fresult));
	}
...
int STATISLINES::InsertResultData( int statis_type, ES_TIME *es_time )
{
	...
	/// @brief 线路终端覆盖率按主站 */
	offset = 0;
	rec_num = m_mOrganCode2TerResult.size();
	m_pdata = (char *)malloc(m_ndata_len * rec_num);
	if (m_pdata == NULL) {
		cout << "malloc error" << endl;
		//plog->WriteLog(2, "malloc error, 文件名：%s, 行号：%d", __FILE__, __LINE__);
	...
	itr_line_result = m_mOrganCode2TerResult.begin();
	for (int i = 0; i < rec_num; i++) {
		memcpy(m_pdata + offset, "tercovline", col_attr[0].data_size);
		offset += col_attr[0].data_size;
		memcpy(m_pdata + offset, &itr_line_result->first, col_attr[1].data_size);
		offset += col_attr[1].data_size;
		memcpy(m_pdata + offset, &itr_line_result->second, col_attr[2].data_size);
	...
	/// @brief 挂接终端线路条数 */
	offset = 0;
	rec_num = m_mOrganCode2HaveTerNum.size();
	m_pdata = (char *)malloc(m_ndata_len * rec_num);
	if (m_pdata == NULL) {
		cout << "malloc error" << endl;
		return -1;
	...
	itr_line_num = m_mOrganCode2HaveTerNum.begin();
	for (int i = 0; i < rec_num; i++) {
		memcpy(m_pdata + offset, "tercovlinenum", col_attr[0].data_size);
		offset += col_attr[0].data_size;
		memcpy(m_pdata + offset, &itr_line_num->first, col_attr[1].data_size);
		offset += col_attr[1].data_size;
		memcpy(m_pdata + offset, &itr_line_num->second, col_attr[2].data_size);
...
void STATISLINES::Clear()
{
	...
	m_mOrganCode2HaveTerNum.clear();
	m_mOrganCode2TerResult.clear();
}
