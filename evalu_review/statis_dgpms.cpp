...
// 			goto FAILED_RET;
...
#if 0
		/// @detail 模型计算时已去重复,不重复操作 */
		ret = DeleteRepeat();
		if (ret < 0) {
			printf("DeleteRepeat fail.\n");
			goto FAILED_RET;
		} // 
#endif
		ret = DeleteBoundArydate();
		if (ret < 0) {
			printf("DeleteBoundArydate fail.\n");
			goto FAILED_RET;
		} // 

		ret = GetDMSCBsByDay(es_time);
		if(ret < 0) {
			printf("GetDMSCBsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSCBsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSCBsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSDISsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSDISsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSDISsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSDISsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSLDsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSLDsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSLDsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSLDsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGpRightsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGpRightsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSGpDevsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSGpDevsByDay fail.\n");
			goto FAILED_RET;
		}
		...
		if(ret < 0)
		{
			printf("GetSyncRightDevsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSStatusDevsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSStatusDevsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSStatusDevsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSStatusDevsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSBUSsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSBUSsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSBUSsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSBUSsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDMSSTsByDay(es_time);
		if(ret < 0)
		{
			printf("GetDMSSTsByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetGPMSSTsByDay(es_time);
		if(ret < 0)
		{
			printf("GetGPMSSTsByDay fail.\n");
			goto FAILED_RET;
		}
...
int StatisDGPMS::GetSyncRightDevsByDay(ES_TIME *es_time)
{
	...
	sprintf(sql, "select a.organ_code,c.dv,count(*) from evalusystem.detail.dmscb a,evalusystem.detail.gpmscb "
		"b,evalusystem.config.organdv c where a.cb=b.cb and a.status=b.status and a.organ_code = b.organ_code and a.flag=1 and c.organ_code = a.organ_code group by a.organ_code,c.dv");
...
int StatisDGPMS::GetDMSStatusDevsByDay(ES_TIME *es_time)
{
	...
	sprintf(sql, "select a.organ_code,c.dv,count(*) from evalusystem.detail.dmscb a,"
		"evalusystem.detail.gpmscb b,evalusystem.config.organdv c where a.cb=b.cb and a.organ_code = b.organ_code and a.flag=1 and a.organ_code = c.organ_code group by a.organ_code,c.dv");
...
// 				err_info.error_no, err_info.error_info, __FILE__, __LINE__);
	...
	sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
		...
		"or code='pmsgpnum' or code='allgpnum') and dimname='无' group by code,organ_code,datatype", 
	...
	sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
		...
		"code='dmsbuss' or code='pmsbuss' or code='dmssts' or code='pmssts') and dimname='无' group by "
	...
	sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
		...
		"or code='substionfine' or code='devfine') and dimname='无'  and datatype != '数值' group by code,organ_code,datatype", 
		byear, bmonth, byear, bmonth, bday, tmp_str.c_str());
...
int StatisDGPMS::GetDMSGPMSRate(int statis_type, ES_TIME *es_time)
{
	...
	itr_organ_gpmsbus = m_organ_gpmsbus.begin();
	for (;itr_organ_gpmsbus != m_organ_gpmsbus.end();++itr_organ_gpmsbus) {

		ES_RESULT es_result;
		es_result.m_ntype = itr_organ_gpmsbus->first;
		itr_organ_dmsbus = m_organ_dmsbus.find(itr_organ_gpmsbus->first);
		if (itr_organ_dmsbus != m_organ_dmsbus.end()) {

			if (itr_organ_dmsbus->second <= itr_organ_gpmsbus->second) {
	...
	itr_organ_gpmsst = m_organ_gpmsst.begin();
	for (;itr_organ_gpmsst != m_organ_gpmsst.end();++itr_organ_gpmsst) {

		ES_RESULT es_result;
		es_result.m_ntype = itr_organ_gpmsst->first;
		itr_organ_dmsst = m_organ_dmsst.find(itr_organ_gpmsst->first);
		if (itr_organ_dmsst != m_organ_dmsst.end()) {
...
int StatisDGPMS::InsertResultSTsRate(int statis_type, ES_TIME *es_time)
{
	...
	map<int,float>::iterator it_dev_result = m_dev_result.begin();
	for(; it_dev_result != m_dev_result.end(); it_dev_result++)
	{
		memcpy(m_pdata + offset, "devfine", col_attr[0].data_size);
		offset += col_attr[0].data_size;
		memcpy(m_pdata + offset, &it_dev_result->first, col_attr[1].data_size);
		offset += col_attr[1].data_size;
		memcpy(m_pdata + offset, &it_dev_result->second, col_attr[2].data_size);
		offset += col_attr[2].data_size;
		memcpy(m_pdata + offset, "百分比", col_attr[3].data_size);
		offset += col_attr[3].data_size;
		memcpy(m_pdata + offset, "无", col_attr[4].data_size);
...
int StatisDGPMS::GetAverageRate( int organ_code,float result )
{
	map<int,float>::iterator it = m_dev_result.find(organ_code);
	if (it != m_dev_result.end()) {

		it->second += result/DIVIDENUM;
	}
	else {

		m_dev_result.insert(make_pair(organ_code,result/DIVIDENUM));
	}

	return 0;
}

int StatisDGPMS::DeleteRepeat()
{
	int ret = 0;
	char sql[64];
	//设备列表去重
...
int StatisDGPMS::DeleteBoundArydate()
{
	int ret = 0;
	char sql[64];
	//设备列表去重
	sprintf(sql, "CALL EVALUSYSTEM.DETAIL.DELETEBOUNDARYDAT()");
