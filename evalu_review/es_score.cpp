...
int ESScore::GetK(const string &code,float v,float &k)
{
	...
	if (rec_num != 0)
	{
		...
		for(i = 0;i < rec_num; i++)
		{
			...
			else if(itr_code_score->second.value > v && itr_code_score->second.value <= maxfloat)
			{
	...
	k = minnum + (v-minfloat)*(maxnum-minnum)/(maxfloat-minfloat);
...
int ESScore::InsertScore(int statis_type, ES_TIME *es_time)
{
	...
	//删除总分旧记录
	if(statis_type == STATIS_DAY) {
		sprintf(sql, "delete from evalusystem.result.day_%04d%02d%02d where code = 'score' and datatype != '得分';", year,mon,day);
	}
	else {
		sprintf(sql, "delete from evalusystem.result.month_%04d%02d where code = 'score' and datatype != '得分';", year,mon);
	}
	...
	if(statis_type == STATIS_DAY) {
		sprintf(sql, "insert into evalusystem.result.day_%04d%02d%02d(code,organ_code,value,datatype,"
			"statistime,updatetime,dimname,dimvalue) (select 'score',a.organ_code,round(sum(a.value*b.weight)/100,4),'数值',"
			...
			"where a.code=b.code))) c,(select organ_code from evalusystem.config.organ where flag = 1) d where a.code = b.code and datatype = '数值' and dimname = '无' and a.code != 'score' "
			"and d.organ_code = a.organ_code group by a.organ_code);", year,mon,day,year,mon,day,year,mon,day,year,mon,day);
	}
	else {
		sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
			...
			"a where settime=(select max(settime) from evalusystem.config.weight b where a.code=b.code))) c,(select organ_code from evalusystem.config.organ "
			"where flag = 1) d where a.code = b.code and "
			"datatype = '数值' and dimname = '无' and a.code != 'score' and d.organ_code = a.organ_code group by a.organ_code);",
	...
	if(statis_type == STATIS_DAY) {
		sprintf(sql, "insert into evalusystem.result.day_%04d%02d%02d(code,organ_code,value,datatype,"
			"statistime,updatetime,dimname,dimvalue) (select 'score',a.organ_code,round(sum(a.value*b.weight)/100,4)+15,'数值',"
			"to_date('%04d-%02d%-02d','YYYY-MM-DD'),sysdate,'无','无' from (select * from evalusystem.result.day_%04d%02d%02d "
			"a where updatetime=(select max(updatetime) from evalusystem.result.day_%04d%02d%02d b where a.code=b.code and "
			"a.organ_code=b.organ_code and  a.datatype=b.datatype and a.dimname=b.dimname and a.dimvalue=b.dimvalue and "
			"datatype = '数值' and code != 'score')) a,(select * from evalusystem.config.weight a where "
			"settime=(select max(settime) from evalusystem.config.weight b where a.code=b.code)  and caflag = 0) b,(select sum(weight) s "
			"from (select * from evalusystem.config.weight a where settime=(select max(settime) from evalusystem.config.weight b "
			"where a.code=b.code))) c,(select organ_code from evalusystem.config.organ where flag = 0) d where a.code = b.code and datatype = '数值' and dimname = '无' and a.code != 'score' "
			"and d.organ_code = a.organ_code group by a.organ_code);", year,mon,day,year,mon,day,year,mon,day,year,mon,day);
	}
	else {
		sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
			...
			"a.organ_code=b.organ_code and  a.datatype=b.datatype and a.dimname=b.dimname and a.dimvalue=b.dimvalue and "
			"datatype = '数值' and code != 'score')) a,(select * from evalusystem.config.weight a where settime=(select max(settime) "
			"from evalusystem.config.weight b where a.code=b.code)  and caflag = 0) b,(select sum(weight) s from (select * from evalusystem.config.weight "
			"a where settime=(select max(settime) from evalusystem.config.weight b where a.code=b.code))) c,(select organ_code from evalusystem.config.organ "
			"where flag = 0) d where a.code = b.code and "
			"datatype = '数值' and dimname = '无' and a.code != 'score' and d.organ_code = a.organ_code group by a.organ_code);",
			year,mon,year,mon,day,year,mon,year,mon);
	}
