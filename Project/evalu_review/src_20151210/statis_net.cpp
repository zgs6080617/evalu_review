...
int StatisNet::InsertNetScore()
{
	...
	sprintf(sql_delete,"delete from evalusystem.manage.err_file where file_name = '网络质量指标' and date like '%s%%';",today);
	...
	if(GetNetValue() == 0)
	{
		...
		sprintf(sql,"insert into evalusystem.manage.err_file(date,organ_code,file_name,dis,source) values "
