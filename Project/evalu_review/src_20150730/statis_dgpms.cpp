...
int StatisDGPMS::GetDMSGPMSRate(int statis_type, ES_TIME *es_time)
{
	...
	itr_organ_dcbs = m_organ_dcbs.begin();
	for(; itr_organ_dcbs != m_organ_dcbs.end(); itr_organ_dcbs++)
	{
		ES_RESULT es_result;
		es_result.m_ntype = itr_organ_dcbs->first;
		itr_organ_gpcbs = m_organ_gpcbs.find(itr_organ_dcbs->first);
		if(itr_organ_gpcbs != m_organ_gpcbs.end())
		{
			if(itr_organ_dcbs->second <= itr_organ_gpcbs->second)
				es_result.m_fresult = itr_organ_dcbs->second / itr_organ_gpcbs->second;
	...
	//按馈线
	itr_dv_dcbs = m_dv_dcbs.begin();
	for(; itr_dv_dcbs != m_dv_dcbs.end(); itr_dv_dcbs++)
	{
		ES_RESULT es_result;
		es_result.m_strtype = itr_dv_dcbs->first;
		es_result.m_ntype = itr_dv_dcbs->second.organ_code;
		itr_dv_gpcbs = m_dv_gpcbs.find(itr_dv_dcbs->first);
		if(itr_dv_gpcbs != m_dv_gpcbs.end())
		{
	...
	//按主站
	itr_organ_ddiss = m_organ_ddiss.begin();
	for(; itr_organ_ddiss != m_organ_ddiss.end(); itr_organ_ddiss++)
	{
		ES_RESULT es_result;
		//ES_RESULT es_score;
		es_result.m_ntype = itr_organ_ddiss->first;
		itr_organ_gpdiss = m_organ_gpdiss.find(itr_organ_ddiss->first);
	...
	//按馈线
	itr_dv_ddiss = m_dv_ddiss.begin();
	for(; itr_dv_ddiss != m_dv_ddiss.end(); itr_dv_ddiss++)
	{
		ES_RESULT es_result;
		es_result.m_strtype = itr_dv_ddiss->first;
		es_result.m_ntype = itr_dv_ddiss->second.organ_code;
		itr_dv_gpdiss = m_dv_gpdiss.find(itr_dv_ddiss->first);
		if(itr_dv_gpdiss != m_dv_gpdiss.end())
		{
	...
	//按主站
	itr_organ_dlds = m_organ_dlds.begin();
	for(; itr_organ_dlds != m_organ_dlds.end(); itr_organ_dlds++)
	{
		ES_RESULT es_result;
		//ES_RESULT es_score;
		es_result.m_ntype = itr_organ_dlds->first;
		itr_organ_gplds = m_organ_gplds.find(itr_organ_dlds->first);
		if(itr_organ_gplds != m_organ_gplds.end())
		{
	...
	//按馈线
	itr_dv_dlds = m_dv_dlds.begin();
	for(; itr_dv_dlds != m_dv_dlds.end(); itr_dv_dlds++)
	{
		ES_RESULT es_result;
		es_result.m_strtype = itr_dv_dlds->first;
		es_result.m_ntype = itr_dv_dlds->second.organ_code;
		itr_dv_gplds = m_dv_gplds.find(itr_dv_dlds->first);
		if(itr_dv_gplds != m_dv_gplds.end())
		{
	...
	//按主站
	itr_organ_rights = m_organ_rights.begin();
	for(; itr_organ_rights != m_organ_rights.end(); itr_organ_rights++)
	{
		ES_RESULT es_result;
		//ES_RESULT es_score;
		es_result.m_ntype = itr_organ_rights->first;
		itr_organ_dgpdevs = m_organ_dgpdevs.find(itr_organ_rights->first);
		if(itr_organ_dgpdevs != m_organ_dgpdevs.end())
	...
	for(; itr_gp_rights != m_gp_dgpdevs.end(); itr_gp_rights++)
	{
		ES_RESULT es_result;
		es_result.m_strtype = itr_gp_rights->first.gp_type;
		es_result.m_ntype = itr_gp_rights->first.organ_code;
		ES_ORGAN_GP es_organ;
		es_organ.organ_code = itr_gp_rights->first.organ_code;
		es_organ.gp_type = itr_gp_rights->first.gp_type;
		...
		itr_gp_dgpdevs = m_gp_dgpdevs.find(es_organ);
		if(itr_gp_dgpdevs != m_gp_dgpdevs.end())
	...
	//按主站
	itr_organ_sync = m_organ_sync.begin();
	for(; itr_organ_sync != m_organ_sync.end(); itr_organ_sync++)
	{
		ES_RESULT es_result;
		//ES_RESULT es_score;
		es_result.m_ntype = itr_organ_sync->first;
		itr_organ_dsdevs = m_organ_dsdevs.find(itr_organ_sync->first);
		if(itr_organ_dsdevs != m_organ_dsdevs.end())
	...
	//按馈线
	itr_dv_sync = m_dv_sync.begin();
	for(; itr_dv_sync != m_dv_sync.end(); itr_dv_sync++)
	{
		ES_RESULT es_result;
		es_result.m_strtype = itr_dv_sync->first;
		es_result.m_ntype = itr_dv_sync->second.organ_code;
		itr_dv_dsdevs = m_dv_dsdevs.find(itr_dv_sync->first);
		if(itr_dv_dsdevs != m_dv_dsdevs.end())
