...
int StatisYC::GetTdTotalNum(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
		...
		"where count_time like '%04d-%02d%%' and (sendtype != 2 or sendtype is null) and flag != 1) a,(select organ_code,substr(dmsid,instr(dmsid,'_',1,1)+1,3) dmsid "
...
int StatisYC::GetGPMSFailureNum(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
		...
		sprintf(sql, "select organ_code,count(*) from evalusystem.detail.gpms_powercut_det where "
			"count_time like '%04d-%02d%%' and flag != 1 group by organ_code", byear, bmonth);
...
int StatisYC::GetDmsNoUseJudged(int statis_type, ES_TIME *es_time)
{
	...
	sprintf(sql, "select a.organ_code,a.tdnum,b.tdnum,to_char(a.judged_time,'YYYY-MM-DD HH24:MI:SS'),to_char(b.judged_time,'YYYY-MM-DD HH24:MI:SS'),a.tdintr,b.tdintr "
		"from (select organ_code,tdnum,judged_time,tdintr from detail.tdinfo where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS')"
		"  and sendtype = 2 and flag != 1) a,(select organ_code,tdnum,judged_time,tdintr from detail.tdinfo where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS')"
		"  and sendtype = 0 and flag != 1) b where a.organ_code = b.organ_code and datediff(second,a.judged_time,b.judged_time) >= 0;",
		year,month,day,year,month,day);
