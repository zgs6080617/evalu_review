...
int ESScore::InsertPointScore(int statis_type, ES_TIME *es_time)
{
	...
	sprintf(qury,"INSERT INTO EVALUSYSTEM.RESULT.%s(CODE,ORGAN_CODE,VALUE,DATATYPE,"
		"STATISTIME,UPDATETIME,DIMNAME,DIMVALUE) (SELECT 'score',A.ORGAN_CODE,ROUND(SUM(A.VALUE*B.WEIGHT/C.S),4),'数值',"
		"TO_DATE('%04d-%02d%-02d','YYYY-MM-DD'),SYSDATE,'无','无' FROM (SELECT * FROM EVALUSYSTEM.RESULT.%s "
		"WHERE DATATYPE = '数值') A,(SELECT * FROM EVALUSYSTEM.CONFIG.WEIGHT A WHERE "
		...
		"WHERE A.CODE=B.CODE))) C WHERE A.CODE = B.CODE AND DATATYPE = '数值' AND DIMNAME = '无' AND A.CODE != 'score' GROUP BY A.ORGAN_CODE);"
