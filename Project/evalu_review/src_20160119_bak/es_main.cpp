...
int main(int argc, char **argv)
{
	...
	for( ; ; )
	{
		...
		if(hour == 9 && min == 0 && weekday == 3)
		{
			...
			GetBeforeweekDate(byear,bmonth,bday,&bwyear,&bwmonth,&bwday);
			...
			sprintf(sql, "call evalusystem.result.DMS_TD_MATCH('%04d-%02d-%02d','%04d-%02d-%02d')", byear, bmonth, bday,bwyear,bwmonth,bwday);
