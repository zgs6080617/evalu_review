...
int StatisDA::GetDARunningRate(int statis_type, ES_TIME *es_time)
{
	...
	if(ret < 0)
	{
		printf("GetDASrcDaRelation fail.\n");
		goto FAILED_RET;
	}
	...
	{	
		...
		if(ret < 0)
		{
			printf("GetDAFinishTimesByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDAStartTimesByDay(es_time);
		if(ret < 0)
		{
			printf("GetDAStartTimesByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDALinesByDay(es_time);
		if(ret < 0)
		{
			printf("GetDALinesByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDADisableTimeByDay(es_time);
		if(ret < 0)
		{
			printf("GetDADisableTimeByDay fail.\n");
			goto FAILED_RET;
		}
		ret = GetDASuccessTimesByDay(es_time);
		if(ret < 0)
		{
			printf("GetDASuccessTimesByDay fail.\n");
			goto FAILED_RET;
		}
#endif
		...
		if(ret < 0)
		{
			printf("GetFATouyunLines fail.\n");
			goto FAILED_RET;
		}
		ret = GetFALines(statis_type, es_time);
		if(ret < 0)
		{
			printf("GetFALines fail.");
			goto FAILED_RET;
		}
	}
	else
	{
		...
		if(ret < 0)
		{
			printf("GetDAFinishTimesByMonth fail.\n");
			goto FAILED_RET;
		}
		ret = GetDAStartTimesByMonth(es_time);
		if(ret < 0)
		{
			printf("GetDAStartTimesByMonth fail.\n");
			goto FAILED_RET;
		}
		ret = GetDALinesByDay(es_time);
		if(ret < 0)
		...
		if(ret < 0)
		{
			printf("GetDADisableTimeByMonth fail.\n");
			goto FAILED_RET;
		}
		ret = GetDASuccessTimesByMonth(es_time);
		if(ret < 0)
		{
			printf("GetDASuccessTimesByMonth fail.\n");
			goto FAILED_RET;
		}
#endif
		ret = GetFATouyunLines(statis_type, es_time);
	...
	if(ret < 0)
	{
		printf("InsertResultDASuccessRate fail.\n");
		goto FAILED_RET;
	}
	ret = InsertResultDAAbleRate(statis_type, es_time);
	if(ret < 0)
	{
		printf("InsertResultDAAbleRate fail.\n");
		goto FAILED_RET;
	}
	ret = InsertResultDAAccuracyRate(statis_type, es_time);
	if(ret < 0)
	{
		printf("InsertResultDAAccuracyRate fail.\n");
		goto FAILED_RET;
	}
#endif
	...
	if(ret < 0)
	{
		printf("InsertResultFATouyunRate fail.\n");
		goto FAILED_RET;
	}
