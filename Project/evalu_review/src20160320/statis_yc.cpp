...
int StatisYC::GetYCRate(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
	{
		...
		if(ret < 0)
		{
			printf("GetRightJudgedNum_intime fail.\n");
			goto FAILED_RET;
		}
...
int StatisYC::GetYCsRate(int statis_type, ES_TIME *es_time)
{
	...
	for(; itr_organ_dmsjudged != map_organ_judgednum.end(); itr_organ_dmsjudged++)
	{
		...
		map<int, float>::iterator itr_organ_nouseno = map_organ_judgedNoUse.find(itr_organ_dmsjudged->first);
		map<int, float>::iterator itr_organ_nouseno_intime = map_organ_judgedNoUse_intime.find(itr_organ_dmsjudged->first); //add by 20160107 for new tdjudged
		//cout << "organ_code:find:" << itr_organ_dmsjudged->first << endl;
		if(itr_organ_nouseno != map_organ_judgedNoUse.end())
		{
			if(itr_organ_dmsjudged->second == 0 && itr_organ_nouseno->second == 0)
			{
				es_result.m_fresult = 1.0;
			}
			else
			{
				es_result.m_fresult = (itr_organ_dmsjudged->second + itr_organ_nouseno_intime->second) / (itr_organ_dmsjudged->second+itr_organ_nouseno->second);
	...
	//按主站
	itr_organ_dmsjudged = map_organ_judgednum.begin();
	for(; itr_organ_dmsjudged != map_organ_judgednum.end(); itr_organ_dmsjudged++)
	{
		ES_RESULT es_result;
		//ES_RESULT es_score;
		es_result.m_ntype = itr_organ_dmsjudged->first;
		map<int, float>::iterator itr_organ_nouseno = map_organ_judgedNoUse.find(itr_organ_dmsjudged->first);
		map<int, float>::iterator itr_organ_dmsjudgedsum = map_organ_judgedsum.find(itr_organ_dmsjudged->first);
		//cout << "organ_code:find:" << itr_organ_dmsjudged->first << endl;
		if(itr_organ_nouseno != map_organ_judgedNoUse.end()&&itr_organ_dmsjudgedsum != map_organ_judgedsum.end())
		{
			if (itr_organ_dmsjudgedsum->second == 0)
				es_result.m_fresult = 1.0;
			else
				es_result.m_fresult = (itr_organ_dmsjudged->second+itr_organ_nouseno->second)/itr_organ_dmsjudgedsum->second;
			//cout << "in:" << "\t1:" << itr_organ_nouseno->second << "\t2:" << itr_organ_dmsjudged->second << endl;
		}
		else if (itr_organ_dmsjudgedsum == map_organ_judgedsum.end())
			es_result.m_fresult = 1.0;
		else
			es_result.m_fresult = itr_organ_dmsjudged->second/itr_organ_dmsjudgedsum->second;
...
int StatisYC::GetYCRateByMonth(ES_TIME *es_time)
{
	...
	//采集成功率
	sprintf(sql, "select organ_code,avg(cj) from evalusystem.detail.ycrate where " 
		"count_time like '%04d-%02d%%' and cjflag !=1 group by organ_code", byear, bmonth);
...
int StatisYC::GetTdTotalNum(int statis_type, ES_TIME *es_time)
{
	...
	//为停电信息统计添加右连接，add by lcm 20150902
		...
		*/
		...
		sprintf(sql, "select organ.organ_code,decode(abc.c,NULL,0,abc.c) from("
		"select a.organ_code,count(*) c from (select organ_code,tdnum from evalusystem.detail.tdinfo "
		"where count_time like '%04d-%02d%%' and (sendtype != 2 or sendtype is null) and flag != 1) a,(select organ_code,substr(dmsid,instr(dmsid,'_',1,1)+1) dmsid "
		"from evalusystem.detail.gpms_powercut_det where count_time like '%04d-%02d%%' and dmsid is not null and flag != 1) b where a.organ_code = b.organ_code and "
		"a.tdnum = b.dmsid group by a.organ_code) abc right join evalusystem.config.organ organ on(organ.organ_code = abc.organ_code);",
		byear, bmonth, byear, bmonth);
...
int StatisYC::GetGPMSFailureNum(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code,count(*) c from evalusystem.detail.gpms_powercut_det where "
			"count_time='%04d-%02d-%02d 00:00:00.0' group by organ_code) a right join evalusystem.config.organ organ on(organ.organ_code = a.organ_code)",
			byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code,count(*) c from evalusystem.detail.gpms_powercut_det where "
...
int StatisYC::InsertResultTDJudgedRate(int statis_type, ES_TIME *es_time)
{
	...
	{
		memcpy(m_pdata + offset, "judgednouse_intime", col_attr[0].data_size);
		offset += col_attr[0].data_size;
		memcpy(m_pdata + offset, &itr_organ_dmsfailure->first, col_attr[1].data_size);
		offset += col_attr[1].data_size;
		memcpy(m_pdata + offset, &itr_organ_dmsfailure->second, col_attr[2].data_size);
...
int StatisYC::InsertResultTDJudgedRateDET(int statis_type, ES_TIME *es_time)
{
	...
	sprintf(sql, "insert into evalusystem.result.DMSJUDGED(organ_code,count_time,dmsid,fault_no,dmsjudged_time,gpmsjudged_time,tdintr,gpms_tdintr,type,judged_type"
		") values(:1,to_date('%04d-%02d-%02d','YYYY-MM-DD'),:2,:3,to_date(:4,'YYYY-MM-DD HH24:MI:SS'),to_date(:5,'YYYY-MM-DD HH24:MI:SS'),:6,:7,:8,:9);",
		year,month,day);
...
int StatisYC::GetGDMSJudged_new(int statis_type, ES_TIME *es_time,int type)
{
	int year, month, day;
	int lyear, lmonth, lday;
	lyear = es_time->year;
	...
	sprintf(sql, "select a.organ_code,a.tdnum,b.fault_no,to_char(a.judged_time,'YYYY-MM-DD HH24:MI:SS'),to_char(b.judged_time,'YYYY-MM-DD HH24:MI:SS'),a.tdintr,b.tdintr,a.judged_type "
		"from (select organ_code,tdnum,judged_time,tdintr,judged_type from detail.tdinfo where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS')"
		"  and sendtype = 2 and flag != 1) a,(select organ_code,dmsid,fault_no,judged_time,tdintr "
		"from detail.gpms_powercut_det where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS') "
		"and flag != 1) b where a.organ_code = b.organ_code;",
		year,month,day,year,month,day);
	...
	sprintf(sql, "select a.organ_code,a.tdnum,b.fault_no,to_char(a.judged_time,'YYYY-MM-DD HH24:MI:SS'),to_char(b.judged_time,'YYYY-MM-DD HH24:MI:SS'),a.tdintr,b.tdintr,a.judged_type "
		...
		"  and sendtype = 2 and flag != 1) a,(select organ_code,dmsid,fault_no,judged_time,tdintr "
		"from detail.gpms_powercut_det where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS') "
		"and flag != 1) b where a.organ_code = b.organ_code and datediff(second,a.judged_time,b.judged_time) >= 0;",
		year,month,day,year,month,day);
	}
...
/*	sprintf(sql,"delete from evalusystem.result.dmsjudged a where rowid!="
		...
		"a.count_time=b.count_time and a.fault_no=b.fault_no and count_time = '%04d-%02d-%02d 00:00:00.0');",
		byear,bmonth,bday);
...
/*
	if(statis_type == STATIS_DAY)
		...
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' order by organ_code,dmsjudged_time) group by organ_code) "
...
*/		
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsjudged_time"
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' and (type = 1) order by organ_code,dmsjudged_time) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsjudged_time"
		" from evalusystem.result.dmsjudged where count_time like '%04d-%02d%%' and (type = 1) and flag != 1) group by organ_code) a right join "
		"evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth);
...
/*
	if(statis_type == STATIS_DAY)
		...
		"from (select distinct organ_code, dmsjudged_time from(select * from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' "
...
*/
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c "
		"from (select distinct organ_code, dmsjudged_time from(select * from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' "
        " and (type = 1) and datediff(minute,dmsjudged_time,gpmsjudged_time) <= 30 order by organ_code, dmsjudged_time)) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c "
		"from (select distinct organ_code,dmsjudged_time from (select * from evalusystem.result.dmsjudged where count_time like '%04d-%02d%%' and (type = 1) and flag != 1"
        " and datediff(minute,dmsjudged_time,gpmsjudged_time) <= 30)) group by organ_code) a right join "
		"evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth);
...
int StatisYC::GetJudgedAll(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
		"count_time='%04d-%02d-%02d 00:00:00.0' and (sendtype = 1 or sendtype is null or sendtype = 2) group by organ_code) a right join evalusystem.config.organ organ on(organ.organ_code = a.co)",
		byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
...
{
	map_judged.clear();
	if(GetGDMSJudged_new(statis_type,es_time,0) < 0)
		return -1;
	if(GetGDMSJudged_new(statis_type,es_time,1) < 0)
		return -1;
