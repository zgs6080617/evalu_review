...
int StatisYD::GetGDMSYDDnum(int statis_type,ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
	{
		...
		"(select organ_code, ydd_id from evalusystem.detail.dms_yd where count_time like '%s%%') a, "
		"(select organ_code, ydd_id from evalusystem.detail.gpms_yd where count_time like '%s%%') b "
		"where a.organ_code = b.organ_code and a.ydd_id = b.ydd_id group by a.organ_code;",bfday,bfday);
	}
	else
	{
		sprintf(sql,"select a.organ_code, count(*) c from "
		"(select organ_code, ydd_id from evalusystem.detail.dms_yd where count_time like '%s%%') a, "
		"(select organ_code, ydd_id from evalusystem.detail.gpms_yd where count_time like '%s%%') b "
		"where a.organ_code = b.organ_code and a.ydd_id = b.ydd_id group by a.organ_code;",bfmonth,bfmonth);
	}
...
int StatisYD::GetGPMSYDDnum(int statis_type,ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
	{
		...
		"(select organ_code,count(*) c from evalusystem.detail.gpms_yd where count_time like '%s%%' group by organ_code) a "
...
{
	map_ydd_mark.clear();
	map<int,float>::iterator it_gpmsydd_num = map_gpmsydd_num.begin();
	for(;it_gpmsydd_num!=map_gpmsydd_num.end();it_gpmsydd_num++)
	{
		float result;
		map<int,float>::iterator it_gdmsydd_num = map_gdmsydd_num.find(it_gpmsydd_num->first);
		if(it_gdmsydd_num != map_gdmsydd_num.end())
		{
			if( it_gpmsydd_num->second == 0)
				result = 0.0;
			else if(it_gdmsydd_num->second == 0)
				result = 0.0;
			else
		...
		map_ydd_mark.insert(make_pair(it_gpmsydd_num->first,result));
	}
	return 0;
}
