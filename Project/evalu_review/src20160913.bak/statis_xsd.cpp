...
int StatisXSD::GetXSDRate(int statis_type,ES_TIME *es_time)
{
	...
	else
	{
		...
		sprintf(sql,"select organ.organ_code,decode(a.c,NULL,0,a.c) from "
			"(select organ_code,count(*) c from evalusystem.detail.ycrate where "
			"count_time like '%s%%' and xsd_rate >= 0.0 and xsd_rate < 0.95 group by organ_code) a "
			"right join config.organ organ on(a.organ_code = organ.organ_code)",bfmonth);
	}
			...
			for(col = 0; col < col_num; col++)
			{
				...
				case 1:
					...
					if (statis_type == STATIS_DAY) {
						...
						m_num = 1-*(int*)(tempstr)*0.001; //指标值低于0.95减去0.01
					}
					break;
				default:
					break;
...
{
	Clear();
	if(GetXSDsum(statis_type,es_time) < 0)
	{
		printf("GetXSDsum failed.\n");
	}
	if(InsertXSD(statis_type,es_time) < 0)
	{
		printf("InsertXSD failed.\n");
	}
	if(GetXSDFailedNum(statis_type,es_time) < 0)
	{
		printf("GetXSDFailedNum failed.\n");
	}
	if(InsertXSDFailedNumData(statis_type,es_time) < 0)
	{
		printf("InsertXSDFailedNumData failed.\n");
	}
	/*if(GetXSDRate(statis_type,es_time) < 0)  //迁移到statis_yc.cpp
	{
		printf("GetXSDRate failed.\n");
	}*/
	/*if(InsertXSDRate(statis_type,es_time) < 0)
	{
		printf("InsertXSDRate failed.\n");
	}*/
}


int StatisXSD::GetXSDFailedNum( int statis_type,ES_TIME *es_time )
{
	map_xsdfailed_num.clear();
	char tempstr[100],bfday[20],bfmonth[10];
	int num;
	int rec_num;
	int col_num;
	...
	if(statis_type == STATIS_DAY)
	{
		sprintf(sql,"select organ_code,count(*) from EVALUSYSTEM.DETAIL.DMS_XSD_DET where COUNT_TIME like '%s%%' group by organ_code",bfday);
	}
	else
	{
		sprintf(sql,"select organ_code,count(*) from EVALUSYSTEM.DETAIL.DMS_XSD_DET where COUNT_TIME like '%s%%' group by organ_code",bfmonth);
	}
	...
	if(m_oci->ReadData(sql, &rec_num, &col_num, &col_attr, &buf, &err_info) > 0)
	{
		...
		for (rec = 0; rec < rec_num; rec++)
		{
			...
			map_xsdfailed_num.insert(make_pair(m_organ_code,m_num));
		}
	}
	else
	{
...
int StatisXSD::InsertXSDFailedNumData( int statis_type,ES_TIME *es_time )
{
	int year, month, day;
	int lyear, lmonth, lday;
	lyear = es_time->year;
	...
	char sql[MAX_SQL_LEN];
	char table_name[MAX_NAME_LEN];
	map<int,float>::iterator it_dmsxsdfail_rate;
	...
	offset = 0;
	rec_num = map_xsdfailed_num.size();
	m_pdata = (char *)malloc(m_ndata_len * rec_num);
	if(m_pdata == NULL)
	{
		cout<<"malloc error"<<endl;
	...
	it_dmsxsdfail_rate = map_xsdfailed_num.begin();
	for(; it_dmsxsdfail_rate != map_xsdfailed_num.end(); it_dmsxsdfail_rate++)
	{
		memcpy(m_pdata + offset, "xsdfailednum", col_attr[0].data_size);
		offset += col_attr[0].data_size;
		memcpy(m_pdata + offset, &it_dmsxsdfail_rate->first, col_attr[1].data_size);
		offset += col_attr[1].data_size;
		memcpy(m_pdata + offset, &it_dmsxsdfail_rate->second, col_attr[2].data_size);
		offset += col_attr[2].data_size;
