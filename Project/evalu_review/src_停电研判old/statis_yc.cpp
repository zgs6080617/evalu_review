...
int StatisYC::InsertResultTDJudgedRateDET(int statis_type, ES_TIME *es_time)
{
	...
	sprintf(sql, "insert into evalusystem.result.DMSJUDGED(organ_code,organ_name,count_time,dmsid,gpmsid,dmsjudged_time,gpmsjudged_time,tdintr,judged_type"
...
int StatisYC::GetRightJudgedNum(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
		...
		"from (select distinct organ_code,dmsjudged_time from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' order by organ_code,dmsjudged_time) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c "
		"from (select distinct organ_code,dmsjudged_time from evalusystem.result.dmsjudged where count_time like '%04d-%02d%%' and flag != 1) group by organ_code) a right join "
