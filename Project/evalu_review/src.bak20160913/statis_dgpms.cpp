...
// 			goto FAILED_RET;
		...
		if (ret < 0)
		{
			printf("InsertResultCorrectivesRate fail.\n");
			goto FAILED_RET;
		}		
	}
	else
...
// 				err_info.error_no, err_info.error_info, __FILE__, __LINE__);
	...
	sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,"
		...
		"or code='substionfine' or code='rectification' or code='devfine') and dimname='无'  and datatype != '数值' group by code,organ_code,datatype", 
		byear, bmonth, byear, bmonth, bday, tmp_str.c_str());
...
int StatisDGPMS::GetDMSCorrectiveByDay(ES_TIME *es_time)
{
	...
	sprintf(sql, "select organ.organ_code,decode(cro.newnum,null,0,cro.newnum),decode(cro.oldnum,null,0,cro.oldnum) from "
		"(select a.organ_code co,a.cb+a.dis+a.st+a.bus+a.ld+a.tz+a.cj+decode(a.topo,NULL,0) newnum,b.cb+b.dis+b.st+b.bus+b.ld+b.tz+b.cj+decode(b.topo,NULL,0) oldnum "
