...
int StatisYC::GetYCRateByDay(ES_TIME *es_time)
{
	...
	sprintf(sql, "select organ.organ_code,decode(a.tz,NULL,0,a.tz),decode(a.cj,NULL,0,a.cj),decode(a.xsd_rate,NULL,0,a.xsd_rate) from "
		...
		"count_time=to_date('%d-%d-%d 0:0:0','YYYY-MM-DD HH24:MI:SS')) a right join config.organ organ on(organ.organ_code = a.organ_code);", byear, bmonth, bday);
...
*/		
	if(statis_type == STATIS_DAY)
		...
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' and (type = 3)  and flag != 1) group by organ_code) "
...
int StatisYC::GetRightJudgedNum_Indicator( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' and (type = 3)  and flag != 1 and judged_type = 3) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday); // type = 1更改为type=3,加入了时间判断
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
...
int StatisYC::GetRightJudgedNum_Trans( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' and (type = 3)  and flag != 1 and judged_type in(1,2,6)) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday); // type = 1更改为type=3,加入了时间判断
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
...
int StatisYC::GetRightJudgedNum_Fa( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
		" from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' and (type = 3)  and flag != 1 and judged_type in(4,5)) group by organ_code) "
		" a right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);", byear, bmonth, bday); // type = 1更改为type=3,加入了时间判断
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c from (select distinct organ_code,dmsid,dmsjudged_time,fault_no"
...
*/
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,null,0,a.c) from (select organ_code,count(*) c "
		"from (select distinct organ_code,dmsid,dmsjudged_time,fault_no from (select * from evalusystem.result.dmsjudged where count_time='%04d-%02d-%02d 00:00:00.0' "
...
int StatisYC::GetJudgedAll_Indicator( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
		"count_time='%04d-%02d-%02d 00:00:00.0' and (sendtype = 1 or sendtype is null or sendtype = 2 ) and judged_type = 3 group by organ_code) a right join evalusystem.config.organ organ on(organ.organ_code = a.co)",
		byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
...
int StatisYC::GetJudgedAll_Trans( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
		"count_time='%04d-%02d-%02d 00:00:00.0' and (sendtype = 1 or sendtype is null or sendtype = 2) and judged_type in(1,2,6) group by organ_code) a right join evalusystem.config.organ organ on(organ.organ_code = a.co)",
		byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
...
int StatisYC::GetJudgedAll_Fa( int statis_type, ES_TIME *es_time )
{
	...
	if(statis_type == STATIS_DAY)
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
		"count_time='%04d-%02d-%02d 00:00:00.0' and (sendtype = 1 or sendtype is null or sendtype = 2) and judged_type in(4,5) group by organ_code) a right join evalusystem.config.organ organ on(organ.organ_code = a.co)",
		byear, bmonth, bday);
	else
		sprintf(sql, "select organ.organ_code,decode(a.c,NULL,0,a.c) from (select organ_code co,count(*) c from evalusystem.detail.tdinfo where "
