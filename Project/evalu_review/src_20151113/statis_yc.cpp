...
int StatisYC::GetYCsRate(int statis_type, ES_TIME *es_time)
{
	...
	for(; itr_organ_dmsjudged != map_organ_judgednum.end(); itr_organ_dmsjudged++)
	{
		...
		if(itr_organ_nouseno != map_organ_judgedNoUse.end())
		{
			...
			else if (itr_organ_dmsjudged->second == 0)
			{
				es_result.m_fresult = 0.0;
			}
			else
...
//DMS研判成功比GPMS发布早对应关系(未做一一对应)
int StatisYC::GetGDMSJudged_new(int statis_type, ES_TIME *es_time)
{
	int year, month, day;
	int lyear, lmonth, lday;
	lyear = es_time->year;
	...
	if(ret == 1)
	{
		...
		for (rec = 0; rec < rec_num; rec++)
		{
			...
			if(GetStringCount(buf_dms,";",buf_gpms,",") == 0)
			{
...
/*int StatisYC::GetDmsNoUseJudged(int statis_type, ES_TIME *es_time)
{
	...
	//DMS、GPMS停电故障
	sprintf(sql, "select a.organ_code,a.tdnum,b.tdnum,to_char(a.judged_time,'YYYY-MM-DD HH24:MI:SS'),to_char(b.judged_time,'YYYY-MM-DD HH24:MI:SS'),a.tdintr,b.tdintr "
		"from (select organ_code,tdnum,judged_time,tdintr from detail.tdinfo where count_time = to_date('%d-%d-%d 00:00:00','YYYY-MM-DD HH24:MI:SS')"
...
int StatisYC::GetStringCount( char *dms_str,const char *dms_ivalue, char *gpms_str,const char *gpms_ivalue)
{
	if(strlen(dms_str) ==0 ||strlen(gpms_str) == 0)
		return -1;
	dms_tdintr.clear();
	gpms_tdintr.clear();
	char tmp_str[200];
	...
	//分隔dms停电范围
	char *e_str;
	e_str=strtok(dms_str,dms_ivalue);
	sprintf(tmp_str,"%s",e_str);
	dms_tdintr.push_back(tmp_str);
	while(e_str!=NULL)
    {
	...
	//分隔gpms停电范围
	e_str=strtok(gpms_str,gpms_ivalue);
	sprintf(tmp_str,"%s",e_str);
	...
	for(vector<string>::iterator it_gpms = gpms_tdintr.begin();it_gpms != gpms_tdintr.end();it_gpms++)
	{
		...
		{
			if(strstr((*it_dms).c_str(),(*it_gpms).c_str()) != NULL)
			{
...
{
	if(GetGDMSJudged_new(statis_type,es_time) < 0)
		return -1;
