...
int StatisZLP::GetDMSZLPnum(int statis_type,ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
	{
		...
		"(select organ_code,count(*) c from evalusystem.detail.dms_zlp where start_time like '%s%%' group by organ_code) a "
...
int StatisZLP::GetGPMSZLPnum(int statis_type,ES_TIME *es_time)
{
	...
	{
		sprintf(sql,"select organ.organ_code,decode(a.c,null,0,a.c) from"
		"(select organ_code,count(*) c from evalusystem.detail.gpms_zlp where start_time like '%s%%' group by organ_code) a "
		"right join evalusystem.config.organ organ on(a.organ_code = organ.organ_code);",bfday);
	}
	else
	{
...
int StatisZLP::SetZLPRate()
{
	...
	for(;it_dmszlp_num!=map_dmszlp_num.end();it_dmszlp_num++)
	{
		...
		if(it_gpmszlp_num != map_gpmszlp_num.end())
		{
			...
			result = 0.0;
		map_zlp_mark.insert(make_pair(it_dmszlp_num->first,result));
	}
	return 0;
}
...
{
	Clear();
	GetDMSZLPnum(statis_type,es_time);
	GetGPMSZLPnum(statis_type,es_time);
	SetZLPRate();
	InsertZLPRate(statis_type, es_time);
}
