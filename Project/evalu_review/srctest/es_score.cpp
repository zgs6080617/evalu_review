...
int ESScore::GetScore()
{
	...
	sprintf(sql,"select code,cast(value as float),score from evalusystem.config.score;");
...
int ESScore::CaculateMark()
{
	...
	for (;itr_code_mark != m_code_mark.end();itr_code_mark++)
	{
		...
		GetK(itr_code_mark->first,itr_code_mark->second.value,itr_code_mark->second.value);

		cout << "code:" << es_mark.code << "\t" << itr_code_mark->first << endl;
...
int ESScore::GetK(const string &code,float v,float &k)
{
	...
	if (rec_num != 0)
	{
		...
		for(i = 0;i < rec_num; i++)
		{
			if(itr_code_score->second.value <= v && itr_code_score->second.value >= minfloat)
			{
...
int ESScore::InsertNum(int statis_type, ES_TIME *es_time)
{
	...
	{
		memcpy(m_pdata + offset, &itr_code_pointmark->second.code, col_attr[0].data_size);
...
int ESScore::InsertScore(int statis_type, ES_TIME *es_time)
{
	...
	if(statis_type == STATIS_DAY)
	{
		sprintf(sql, "insert into evalusystem.result.day_%04d%02d%02d(code,organ_code,value,datatype,updatetime,"
			"statistime,dimname,dimvalue) (select 'score',a.organ_code,sum(round(a.value*b.weight*100,4)),'数值',to_date('%04d-%02d%-02d',"
			"'YYYY-MM-DD'),sysdate,'无','无' from result.day_%04d%02d%02d a,config.weight b where a.code = b.code "
			"and datatype = '数值' and dimname = '无' group by a.organ_code);", year,mon,day,year,mon,day,year,mon,day);
	}
	else
	{
		sprintf(sql, "insert into evalusystem.result.month_%04d%02d(code,organ_code,value,datatype,updatetime,"
			"statistime,dimname,dimvalue) (select 'score',a.organ_code,sum(round(a.value*b.weight*100,4)),'数值',"
			"to_date('%04d-%02d-%02d','YYYY-MM-DD'),sysdate,'无','无' from result.month_%04d%02d a,config.weight b where "
			"a.code = b.code and datatype = '数值' and dimname = '无' group by a.organ_code);", year,mon,year,mon,day,year,mon);
	}
